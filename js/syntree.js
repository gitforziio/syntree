...
var debug = true;
var margin = 16; // Number of pixels from tree to edge on each side.
...
Node.prototype.set_width_svg = function(draw, font_size, vert_space, hor_space, term_font, nonterm_font) {
  this.font = term_font;
  if (this.has_children)
    this.font = nonterm_font;

  // var val_width = ctx.measureText(this.value).width;
  var val_width = draw.symbol().text(this.value).font({
    family: this.font,
    size: font_size+'pt',
    anchor: 'middle'
  }).bbox().width;

  // console.log(val_width)

  for (var child = this.first; child != null; child = child.next)
    child.set_width_svg(draw, font_size, vert_space, hor_space, term_font, nonterm_font);
...
Node.prototype.assign_location_svg = function(x, y, font_size, term_lines) {
  // floor + 0.5 for antialiasing
  this.x = Math.floor(x) + 0.5;
  this.y = Math.floor(y) + 0.5;
  ...
  if (this.has_children) {
    var left_start = x - (this.step)*((this.children.length-1)/2);
    for (var i = 0; i < this.children.length; i++)
      this.children[i].assign_location_svg(left_start + i*(this.step), y + vert_space, font_size, term_lines);
  } else {
    if ((this.parent) && (!term_lines) && (this.parent.children.length == 1) && (!this.draw_triangle))
      this.y = this.parent.y + padding_above_text + padding_below_text + font_size;
  }
...
Node.prototype.draw_svg = function(draw, font_size, term_font, nonterm_font, color, term_lines) {
  this.font = term_font;
  if (this.has_children)
    this.font = nonterm_font;
  ...
  draw.text(this.value).font({
    family: this.font,
    size: font_size+'pt'
  }).fill(this.fill).cx(this.x).cy(this.y-font_size*0.5)

  for (var child = this.first; child != null; child = child.next)
    child.draw_svg(draw, font_size, term_font, nonterm_font, color, term_lines);
  ...
  if (this.draw_triangle) {
    draw.polygon([
      ...
      [this.x - this.left_width, this.y - font_size - padding_above_text], 
      [this.x + this.right_width, this.y - font_size - padding_above_text], 
      [this.parent.x, this.parent.y + padding_below_text]
  ...
  draw.line(
    this.parent.x, 
    this.parent.y + padding_below_text, 
    this.x, 
...
MovementLine.prototype.draw_svg = function(draw) {
  var tail_x = this.tail.x + 3;
  this.dest_x -= 3;
  if (this.leftwards) {
    tail_x -= 6;
  ...
  var mx = tail_x
  var my = this.tail.y + padding_below_text
  var q1x1 = tail_x
  var q1y1 = this.bottom_y
  var q1x = (tail_x + this.dest_x) / 2
  var q1y = this.bottom_y
  var q2x1 = this.dest_x
  var q2y1 = this.bottom_y
  var q2x = this.dest_x
  var q2y = this.dest_y + padding_below_text
  ...
  draw.path(pathstr).fill('none').stroke({ width: 1.25 })

  draw.polygon([
    [q2x, q2y], 
    [this.dest_x + 3, this.dest_y + padding_below_text + 10], 
    [this.dest_x - 3, this.dest_y + padding_below_text + 10], 
...
function go_svg(wrapid, str, font_size, term_font, nonterm_font, vert_space, hor_space, color, term_lines) {
  // Clean up the string
  str = str.replace(/^\s+/, "");
  var open = 0;
  ...
  try {
    drawwrap = SVG('drawing')
    draw = drawwrap.nested()
  } catch (err) {
    // throw err
    throw "div.drawing";
  }
  ...
  root.set_width_svg(draw, font_size, vert_space, hor_space, term_font, nonterm_font);
  root.assign_location_svg(0, 0, font_size, term_lines);
  root.find_height();
  ...
  draw.move(x_shift,y_shift)

  root.draw_svg(draw, font_size, term_font, nonterm_font, color, term_lines);

  for (var i = 0; i < movement_lines.length; i++)
    if (movement_lines[i].should_draw) movement_lines[i].draw_svg(draw);
